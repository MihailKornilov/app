...
//							'<div class="icon icon-set mtm2 ml5 pl'._tooltip('Параметры файла', -56).'</div>'.
		...
		case 29:
			/*
                ...
                num_1 - id диалога, через который вносятся данные выбираемого списка [24]
                txt_1 - текст, когда запись не выбрана
                ...
                txt_5 - фильтр для отображения особых значений (зависит от num_1) [40]
				num_2 - возможность добавления новых значений (через диалог)
				num_3 - поиск значений вручную
				num_4 - блокировать выбор
				num_5 - учитывать уровни
				num_6 - значение по умолчанию
				num_7 - автоматическое внесение записи, если отсутствует подобное текстовое значение
			*/
		...
		case 31:
			/*
				num_1 - id диалога - список, из которого будут выбираться галочки
				num_2 - id элемента - содержание
			*/
		...
		case 40:
			/*
				num_1 - id элемента - путь к списку [13]
				txt_1 - текст нулевого значения
		...
		case 44:
			/*
				txt_2 - ids дочерних элементов
				настройка значений через PHP12_44_setup
			*/
			return PHP12_44_print($el, $prm['unit_get']);
		...
		case 54:
			/*
				num_1 - id элемента, указывающего на привязанный список
				num_3 - включение счётчика
				txt_1 - фильтр
			*/
			return $el['name'];
		...
		case 59:
			/*
				txt_1 - текст кнопки
				txt_5 - НЕ ЗАНИМАТЬ (используется под фильтр в [29])
		...
		case 60:
			/*
				...
				num_8 - закруглённые углы (настройка стилей)
			*/
		...
		case 68:
			/*
				num_8 - показывать историю записи, которую принимает текущая страница или диалог
			*/
		...
		case 72:
			/*
				num_1 - id элемента - список, на который происходит воздействие
				num_2 - id элемента - путь к сумме для подсчёта по каждому месяцу
			*/
		...
		case 74:
			/*
				num_1 - id элемента-список, к которому применяется фильтр
