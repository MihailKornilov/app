...
/* ---=== ПРИГЛАШЕНИЕ ПОЛЬЗОВАТЕЛЯ ПО ССЫЛКЕ ===---
	...
	3. Пользователь-получатель заходит по ссылке.
			Функция _userInviteCookieSave сохраненяет кеш в куку 'invite_hash', если пользователь не авторизирован. После регистрации или регистрации этот хеш будет получен.
...
function PHP12_user_invite() {//ссылка на приглашение для пользователя
	...
	'<div class="prel">'.
		...
		'<input type="text"'.
			  ...
			  ' readonly'.
			  ' value="http'.(LOCAL ? '://nyandoma/app' : 's://app.gim-system.ru').'/index.php?invite='.$hash.'"'.
		'/>'.
	'</div>';
}
...
function _userInviteDlgOpen() {//автоматическое открытие диалога для приглашения
	...
	_cookie('invite_submit', $hash, 600);
