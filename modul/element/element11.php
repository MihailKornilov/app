...
function PHP12_v_choose_43hint($BL, $obj_id) {//выплывающая подсказка
	...
	if($elem_id = $hint['element_id']) {
		if(!$EL = _elemOne($elem_id)) {
			$sql = "SELECT *
					FROM `_element`
					WHERE `id`=".$elem_id;
			if(!$EL = query_assoc($sql))
				return '[hint2] Не получены данные элемента, к которому прикреплена подсказка';
			if(!$pid = $EL['parent_id'])
				return '[hint2] Не получены данные родительского элемента, к которому прикреплена подсказка';

			$sql = "SELECT *
					FROM `_element`
					WHERE `id`=".$pid;
			if(!$EL = query_assoc($sql))
				return '[hint3] Не получены данные элемента, к которому прикреплена подсказка';
			if(!$pid = $EL['parent_id'])
				return '[hint4] Не получены данные родительского элемента, к которому прикреплена подсказка';

			$sql = "SELECT *
					FROM `_element`
					WHERE `id`=".$pid;
			if(!$EL = query_assoc($sql))
				return '[hint5] Не получены данные элемента, к которому прикреплена подсказка';
			if($EL['dialog_id'] != 23)
				return '[hint6] Не получены данные родительского элемента, к которому прикреплена подсказка';
