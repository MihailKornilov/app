...
function _dialogOpenVal($dialog_id, $prm, $unit_id_send) {//получение параметров открытия диалога для кнопки или блока
	...
	//если используется дочерний диалог, обязательно только редактирование данных
...
*/
	...
	if(!$dialog = _dialogQuery($dialog_id))
		return _emptyMin10('Диалога не существует, который вносит данные списка.');
...
function PHP12_spisok_cond($el, $unit) {
	...
	if(!$BL = _blockOne($block_id))
		return _emptyMin10('Исходного блока '.$block_id.' не существует.');
	if(!$EL = $BL['elem'])
		return _emptyMin10('Настройка условий будет доступна после вставки списка в блок.');
	if($EL['dialog_id'] != 14)
		return _emptyMin10('Элемент не является списком.');
	if(!$dialog_id = $EL['num_1'])
		return _emptyMin10('Отсутствует ID диалога.');
	if(!$DLG = _dialogQuery($dialog_id))
		return _emptyMin10('Диалога '.$dialog_id.' не существует.');
