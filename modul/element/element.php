...
/*
	Каждый элемент должен пройти проверку:
		...
		7. Title элемента
		8. Вывод через элемент [11]
		9.
*/

function _elementType($type, $el=array(), $dop=array()) {//все возможные варианты манипуляций
	switch($type) {
		...
		//дочерние элементы или значения
...
function _elementCFS($el) {//стили Color Font Size
	return array(
		'color' => $el['color'],
		'font'  => $el['font'],
		'size'  => _num($el['size'])
...
function _element11_struct($el, $prm) {
	/*
...
/* [12] Функция PHP (SA) */
function _element12_struct($el, $prm) {
	/*
...
function _element17($type, $el, $prm) {
	/*
...
/* [18] Dropdown */
function _element18_struct($el, $prm) {
	/*
...
function _element21($type, $el, $prm) {
	if($type == 'struct')
		return array(
...
/* [35] Count: количество */
function _element35($type, $el, $prm) {
	if($type == 'struct')
		return array(
...
function _element44_struct($el, $prm) {
	...
	*/
	return _elementStruct($el) + _elementCFS($el);
}
function _element44_vvv($el, $cl) {
	return array(
		'id'        => _num($cl['id']),
		'dialog_id' => _num($cl['dialog_id']),
		'txt_1'     => $cl['txt_1'],      //для [10]
...
function _element54($type, $el, $prm) {
	if($type == 'struct')
		return array(
...
function _element55($type, $el, $prm) {
	/*
...
function _element57_vvv($el, $cl) {//пункты меню
	return array(
		'id' => _num($cl['id']),
...
/* [62] Фильтр: галочка */
function _element62($type, $el, $prm) {
	if($type == 'struct')
		return array(
			'txt_1'   => $el['txt_1'],      //текст для галочки
...
function _element66($type, $el, $prm) {
	if($type == 'struct')
		return array() + _elementStruct($el);
...
function _element68($type, $el, $prm) {
	if($type == 'struct')
		return array(
...
function _element69($type, $el, $prm) {
	if($type == 'struct')
		return array() + _elementStruct($el);
...
/* [70] Выбор цвета фона */
function _element70($type, $el, $prm) {
	if($type == 'struct')
		return array() + _elementStruct($el);
...
/* [74] Фильтр: Radio */
function _element74($type, $el, $prm) {
	/*
...
/* [77] Фильтр: календарь */
function _element77($type, $el, $prm) {
	if($type == 'struct')
		return array(
			'num_1'   => _num($el['num_1']),//id элемента, размещающего список
...
/* [78] Фильтр: меню */
function _element78($type, $el, $prm) {
	if($type == 'struct')
		return array(
			'num_1'   => _num($el['num_1']),//id элемента, размещающего список
