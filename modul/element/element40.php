...
function _elem40dss_dialog($prm, $dss) {//получение id диалога из дочернего диалога
	if($dss)
		return $dss;
	if(!$bl = _elem40dss_bl($prm))
		return 0;
	...
	return $DLG['dialog_id_parent'];
}
function _elem40dss_hint($prm, $dss) {//получение id диалога из подсказки
	if($dss)
		return $dss;
	if(!$bl = _elem40dss_bl($prm))
		return 0;
...
function _elem40dss_el($prm, $dss) {//получение id диалога на основании элемента
	if($dss)
		return $dss;
	if(!$el = _elemOne($prm['srce']['element_id']))
		return 0;
...
function _40cond_col($r) {//получение имени колонки
	...
	if(!$ell = _elemOne($elem_id))
		return "[40] элемента ".$elem_id." не существует";
	if(!$col = _elemCol($ell))
		return "[40] отсутствует имя колонки";
	if(!$BL = _blockOne($ell['block_id']))
		return "[40] элемент не содержится блок";
	if($BL['obj_name'] != 'dialog')
		return "[40] элемент не из диалога";
	if(!$DLG = _dialogQuery($BL['obj_id']))
